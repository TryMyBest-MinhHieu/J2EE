...
public class CommentController {
    ...
    public boolean addCmt(Comment cmt){
        try {
            String sql = "insert into Comment(CmtContent, PostID, AccID, DateTime) values(?,?,?,?)";
            PreparedStatement stm = con.prepareStatement(sql);
            stm.setNString(1, cmt.getContent());
            stm.setInt(2, cmt.getPostID());
            stm.setInt(3, cmt.getAccID());
            stm.setNString(4, cmt.getDate_time());
    ...
    public List<Comment> getCommentOfPost(int postID){
        try {
            ...
            while(rs.next()){
                ...
                Comment cmt = new Comment(cmtID, cmtContent, postID, accID, datetime);
                cmts.add(cmt);
            }
            return cmts;
        } catch (SQLException ex) {
